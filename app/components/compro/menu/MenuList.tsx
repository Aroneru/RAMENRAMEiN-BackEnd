...
export default function MenuList({ category }: MenuListProps) {
  ...
  const loadShowPriceSetting = async () => {
    try {
      ...
      setShowPrice(true); // Default to showing prices on error
    }
  };
  ...
  const loadMenuItems = async () => {
    try {
      ...
      // Sort: Special ramen items come first, then regular items
      const sortedData = data.sort((a, b) => {
        // Prioritize special ramen
        if (a.is_special_ramen && !b.is_special_ramen) return -1;
        if (!a.is_special_ramen && b.is_special_ramen) return 1;
        
        // Then sort by name starting with "Ramen"
  ...
  return (
    ...
    >
      ...
      {selectedItem && (
        ...
        >
          <div
            className="bg-[#1a1a1a] rounded-2xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto relative"
